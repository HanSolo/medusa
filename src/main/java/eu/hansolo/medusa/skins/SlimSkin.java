...
public class SlimSkin extends SkinBase<Gauge> implements Skin<Gauge> {
    ...
    private void registerListeners() {
        getSkinnable().widthProperty().addListener(new InvalidationListener() {
            @Override public void invalidated(Observable o) {SlimSkin.this.handleEvents("RESIZE");}
        });
        getSkinnable().heightProperty().addListener(new InvalidationListener() {
            @Override public void invalidated(Observable o) {SlimSkin.this.handleEvents("RESIZE");}
        });
        getSkinnable().setOnUpdate(new UpdateEventListener() {
            @Override public void onUpdateEvent(UpdateEvent e) {SlimSkin.this.handleEvents(e.eventType.name());}
        });
