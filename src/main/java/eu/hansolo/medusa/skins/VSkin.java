...
public class VSkin extends SkinBase<Gauge> implements Skin<Gauge> {
    ...
    public VSkin(Gauge gauge) {
        ...
        mouseHandler = new EventHandler<MouseEvent>() {
            @Override public void handle(MouseEvent event) {VSkin.this.handleMouseEvent(event);}
        };
        updateMarkers();
    ...
    private void registerListeners() {
        getSkinnable().widthProperty().addListener(new InvalidationListener() {
            @Override public void invalidated(Observable o) {VSkin.this.handleEvents("RESIZE");}
        });
        ...
        getSkinnable().getMarkers().addListener((ListChangeListener<Marker>) new ListChangeListener<Marker>() {
            @Override public void onChanged(Change<? extends Marker> c) {
                VSkin.this.updateMarkers();
                VSkin.this.redraw();
            }
        });

        getSkinnable().setOnUpdate(new UpdateEventListener() {
            @Override public void onUpdateEvent(UpdateEvent e) {VSkin.this.handleEvents(e.eventType.name());}
        });
        getSkinnable().currentValueProperty().addListener(new InvalidationListener() {
            @Override public void invalidated(Observable e) {VSkin.this.rotateNeedle(getSkinnable().getCurrentValue());}
        });
